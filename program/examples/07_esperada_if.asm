.data
    g_var1: .word 0                  # global var1: integer
    g_var2: .word 0                  # global var2: integer
    g_var3: .word 0                  # global var3: integer
    __nl: .asciiz "\n"
...
main:
    ...
    li $t0, 0
    move $s2, $t0                    # var3 = 0
    slt $t0, $s0, $s1                # t0 = var1 < var2
    sw $s0, g_var1                   # spill/write-back de var
    sw $s1, g_var2                   # spill/write-back de var
    sw $s2, g_var3                   # spill/write-back de var
    sw $t0, -4($fp)                  # spill/write-back de temp
    beq $t0, $zero, L0_else
    lw $s0, g_var1                   # cargar var1
    move $s1, $s0                    # var3 = var1
    sw $s1, g_var3                   # spill/write-back de var
    j L1_endif
L0_else:
    lw $s0, g_var2                   # cargar var2
    move $s1, $s0                    # var3 = var2
    sw $s1, g_var3                   # spill/write-back de var
L1_endif:
    lw $s0, g_var3                   # cargar var3
    mul $t0, $s0, $s0                # t1 = var3 * var3
    move $s0, $t0                    # var3 = t1
    move $a0, $s0
    ...
    li $v0, 4
    syscall
    sw $s0, g_var3                   # spill/write-back de var
